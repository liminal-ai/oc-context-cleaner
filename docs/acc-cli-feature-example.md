# Feature: ac-context-cleaner

This specification defines the complete requirements for ac-context-cleaner, a CLI tool that edits or clones AgentCore session transcripts with intelligent tool call stripping. Edit-in-place is the primary mode; clone is a fallback. It serves as the source of truth for the Tech Lead's design work.

---

## User Profile

**Primary User:** AgentCore agent (e.g., Scout) or human operator
...
After this feature ships, an AgentCore agent or human operator can reduce context pressure on a running session by stripping old tool calls from the session transcript. The tool operates directly on the JSONL session files that AgentCore uses, with edit-in-place as the primary mode (the agent continues in the same session) and clone as a fallback (a new session is created from the cleaned transcript). Before cleaning, users can list available sessions and analyze their composition to choose the right stripping preset. Edit operations are automatically backed up and restorable. Both human-readable and JSON output modes are supported, making the tool equally usable by operators at a terminal and agents invoking it programmatically.
...
ac-context-cleaner delivers a CLI tool for context-aware session cleaning. The tool reduces context consumption by intelligently stripping tool calls based on recency, while preserving session structure and identity continuity.
...
- Patching into AgentCore core (future: configurable params in AgentCore itself)
- ~~Thinking block removal (AgentCore doesn't have thinking blocks like Claude Code)~~ **DEFECT:** Research gap — AgentCore sessions DO contain thinking blocks. Anthropic API rejects them on replay. Resolved during implementation: thinking blocks are now stripped alongside tool calls.
...
|----|------------|--------|-------|-------|
| A1 | AgentCore stores sessions in `~/.agentcore/agents/{agentId}/sessions/` | Validated | Tech Lead | Confirmed via tech research |
...
| A7 | AgentCore reloads session changes via cache invalidation | Validated | Tech Lead | 45s TTL + mtime check; edit-in-place confirmed viable |
| A8 | Session reload uses 45s TTL cache with mtime invalidation | Validated | Tech Lead | No file watcher needed — changes detected when cache expires |
| A9 | Single active session per agent, most-recently-modified = current | Validated | Tech Lead | Most-recently-modified file is the current session |
| A10 | JSONL format uses toolCall/toolResult (not tool_use/tool_result), linear transcript | Validated | Tech Lead | Algorithm logic from session-cloner transfers; field names differ. Session header is separate `{type: "session"}` entry |
| A11 | Agent directory at ~/.agentcore/agents/{agentId}/sessions/, default agent "main" | Validated | Tech Lead | Override via `--agent` flag or `AGENTCORE_AGENT_DIR` env var |
| A12 | Current session detection via most-recently-modified .jsonl heuristic | Validated | Tech Lead | `ls -t *.jsonl | head -1`; confirmed by Scout as expected approach. `--session <id>` overrides when specified |

---

## Flows & Requirements

### 1. Session Discovery (List)

The list command is the entry point for any cleaning workflow. Before an agent or operator can edit or analyze a session, they need to find its ID. This flow is especially critical for agents who must self-invoke the tool without human guidance — they need a programmatic way to discover what sessions exist and identify the right target.

1. User invokes `acc list`
...
**AC-1.4:** `acc list -n <count>` limits output to specified number of sessions

- **TC-1.4a: Limit flag restricts output**
  - Given: 10 sessions exist
  - When: User runs `acc list -n 3`
  - Then: Only 3 sessions displayed

**AC-1.5:** `acc list --json` outputs session list as JSON array

- **TC-1.5a: JSON output is parseable**
  - Given: Sessions exist
  - When: User runs `acc list --json`
  - Then: Output is valid JSON array with session objects

**AC-1.6:** Partial session ID matching supported (like session-cloner)

- **TC-1.6a: Partial ID matching works**
  - Given: Session with ID "abc123-def456-..."
  - When: User runs `acc info abc123`
...
- **TC-1.6b: Ambiguous partial ID fails gracefully**
  ...
  - When: User runs `acc info abc`
  - Then: Error message lists matching sessions

**AC-1.7:** Tool auto-detects active agent from AgentCore environment (env var or process context)

- **TC-1.7a: Agent auto-detected from environment**
  - Given: Tool is run within an AgentCore agent context
  - When: User runs `acc list` without --agent flag
...
- **TC-1.8a: Agent flag overrides auto-detection**
  - Given: Active agent is "scout" but user specifies --agent other-agent
  - When: User runs `acc list --agent other-agent`
...
- **TC-1.9a: Missing agent context shows actionable error**
  - Given: Tool is run outside AgentCore context and no --agent flag
  - When: User runs `acc list`
  - Then: Error message explains agent context required
  - And: Lists available agents if discoverable

### 2. Session Analysis (Info)

Understanding what is in a session — how many messages, how many tool calls, how large it is — determines which stripping preset to use. Running `acc info` before `acc edit` is the expected pattern: analyze first, then act with the right level of aggression.

1. User invokes `acc info <sessionId>`
2. System locates and parses session file
3. System analyzes: turn count, message count, tool call count, file size
4. System estimates token count (using character-based heuristic per session-cloner approach)
5. System outputs analysis (human-readable or JSON)
6. User decides which preset to use based on analysis

#### Acceptance Criteria

**AC-2.1:** `acc info <sessionId>` displays session statistics

- **TC-2.1a: Info displays session statistics**
  - Given: Valid session ID
  - When: User runs `acc info <sessionId>`
...
- **TC-2.3a: Token estimation displayed**
  ...
  - And: Estimation uses character-based heuristic (per session-cloner)
...
**AC-2.5:** `acc info --json` outputs statistics as JSON object

- **TC-2.5a: Info JSON output is complete**
  - Given: Valid session
  - When: User runs `acc info <sessionId> --json`
...
- **TC-2.6a: Error on invalid session ID**
  ...
  - When: User runs `acc info nonexistent`
...
### 3. Edit Session in Place (Primary)

Editing in place is the primary workflow because it lets the agent continue in the same session — no context switch, no new session ID, no disruption. The agent invokes `acc edit`, the tool backs up the session, strips old tool calls, writes the cleaned transcript back, and AgentCore picks up the changes on its next cache refresh (45s TTL with mtime check). This is the routine self-maintenance path.

1. User invokes `acc edit <sessionId> --strip-tools` (or `acc edit --strip-tools` to auto-detect current session)
...
**AC-3.1:** `acc edit <sessionId> --strip-tools` modifies the session file in place

- **TC-3.1a: Edit modifies session in place**
  - Given: Valid session with tool calls
  - When: User runs `acc edit <sessionId> --strip-tools`
...
- **TC-3.5a: Edit JSON output is complete**
  ...
  - When: User runs `acc edit <sessionId> --strip-tools --json`
  - Then: Output is valid JSON matching EditResult interface
  - And: Includes statistics and backup path

**AC-3.6:** Partial session ID matching supported (like session-cloner)

- **TC-3.6a: Partial ID matching works for edit**
  - Given: Session with ID "abc123-def456-..."
  - When: User runs `acc edit abc123 --strip-tools`
...
- **TC-3.7a: Session resumable after edit**
  ...
  - And: AgentCore can continue without restart (mechanism per Tech Design)

**AC-3.8:** `acc edit` (without session ID) auto-detects current session for agent self-invocation

- **TC-3.8a: Edit auto-detects current session**
  - Given: Agent is running in an AgentCore session
  - When: User runs `acc edit --strip-tools` (no session ID)
  - Then: Current session is detected and edited
- **TC-3.8b: Auto-detect fails gracefully outside session**
  - Given: Tool is run outside an AgentCore session context
  - When: User runs `acc edit --strip-tools` (no session ID)
  - Then: Error message indicates session ID required or detection failed

### 4. Clone Session (Fallback)

Cloning creates a new session from an existing one. This is the fallback when edit-in-place is not viable (e.g., the agent needs to archive the original) or when the operator wants a clean copy for experimentation. Clone supports an optional custom output path (`-o`) for backup/export use cases, and a `--no-register` flag to skip session index updates when the clone is purely for external archival.

1. User invokes `acc clone <sessionId> --strip-tools`
...
When a custom output path is specified (`-o /path/to/backup.jsonl`), the system writes to that path instead of the default sessions directory. The session is still registered in the index unless `--no-register` is also provided.

#### Acceptance Criteria

**AC-4.1:** `acc clone <sessionId>` creates a new session file with new UUID

- **TC-4.1a: Clone creates new session with new UUID**
  - Given: Valid source session
  - When: User runs `acc clone <sessionId>`
...
- **TC-4.5a: Custom output path respected**
  - Given: Valid session
  - When: User runs `acc clone <id> -o /tmp/backup.jsonl`
...
- **TC-4.6a: Clone JSON output is complete**
  ...
  - When: User runs `acc clone <sessionId> --json`
...
- **TC-4.7a: Clone without stripping preserves all content**
  ...
  - When: User runs `acc clone <id>` (no --strip-tools)
  - Then: Cloned session contains identical content to original
  - And: Only session ID differs

**AC-4.8:** Partial session ID matching supported (like session-cloner)

- **TC-4.8a: Partial ID matching works for clone**
  - Given: Session with ID "abc123-def456-..."
  - When: User runs `acc clone abc123`
...
- **TC-4.10a: No-register flag skips index update**
  - Given: Valid session
  - When: User runs `acc clone <id> --no-register`
  - Then: Session index is not modified

### 5. Tool Call Stripping

Tool stripping is the core value proposition that cuts across both edit and clone operations. It reduces context consumption while preserving recent tool context so the agent retains awareness of its most recent actions. For technical reasons (avoiding degradation over repeated operations), stripping operates on turn counts rather than percentages. The presets — default, aggressive, extreme — give users a simple vocabulary for choosing how much to strip without needing to understand the underlying mechanics.
...
- **TC-5.1a: Default preset keeps last 20 turns-with-tools**
  ...
  - When: User runs `acc edit <id> --strip-tools`
...
- **TC-5.2a: Aggressive preset keeps last 10 turns-with-tools**
  ...
  - When: User runs `acc edit <id> --strip-tools=aggressive`
...
- **TC-5.3a: Extreme preset removes all tool calls**
  ...
  - When: User runs `acc edit <id> --strip-tools=extreme`
  - Then: Session contains no tool calls or tool results

**AC-5.4:** After stripping, transcript contains no dangling tool_use references that would cause AgentCore parse errors

- **TC-5.4a: Edited session loads without errors**
  - Given: Session with tool calls, some in removal zone
  - When: Edit completes
  - Then: AgentCore can load the edited session without parse errors
  - And: No runtime errors from dangling tool references (exact constraints per Tech Design verification of AgentCore's JSONL semantics)

**AC-5.5:** Most recent turns-with-tools are preserved at full fidelity

**AC-5.6:** Truncation reduces tool call arguments and results to 2 lines or 120 characters, whichever is shorter (per session-cloner)
...
Edit operations automatically create backups so that aggressive stripping is never a one-way door. Backups use monotonic numbering with rotation, and the restore command recovers from the most recent backup. Error recovery is built into this flow: if an edit fails mid-operation, the backup remains intact, no partial files are written, and the original session is unchanged.
...
**AC-6.3:** `acc restore <sessionId>` restores session from most recent backup

- **TC-6.3a: Restore recovers from backup**
  - Given: Session that was previously edited (backup exists)
  - When: User runs `acc restore <sessionId>`
...
Output must serve two audiences with different needs. Human operators want readable formatting with context — session IDs, statistics summaries, backup locations, and clear error messages that tell them what to do next. Agents invoking the tool programmatically need structured JSON they can parse and act on. Both audiences benefit from consistent exit codes and actionable error messages.
...
- **TC-7.6a: Error messages are actionable**
  - Given: Session ID that doesn't exist
  - When: User runs `acc edit nonexistent --strip-tools`
  - Then: Error message explains what went wrong ("Session not found")
  - And: Suggests resolution ("Use 'acc list' to see available sessions")

**AC-7.7:** `--help` displays usage information

- **TC-7.7a: Help flag shows usage**
  - Given: User runs `acc --help`
  - Then: Usage information displayed

**AC-7.8:** `--quickstart` displays condensed agent-friendly help (~250 tokens, following session-cloner pattern: when to use, presets, common commands)

- **TC-7.8a: Quickstart shows condensed help**
  - Given: User runs `acc --quickstart`
  - Then: Agent-friendly condensed help displayed (~250 tokens)
  - And: Includes: when to use, presets, common commands

### 8. Configuration

Configuration support lets users customize behavior without passing flags on every invocation. The precedence chain — CLI flags override environment variables, which override config file values — follows standard CLI conventions and matches the session-cloner pattern.

#### Acceptance Criteria

**AC-8.1:** Tool reads config from standard locations (following session-cloner pattern)

- **TC-8.1a: Config read from standard location**
  - Given: Config file exists at standard location
  - When: User runs any acc command
...
- **TC-8.2a: Custom preset from config applied**
  ...
  - When: User runs `acc edit <id> --strip-tools=conservative`
...
- **TC-8.3a: Environment variable overrides config**
  ...
  - And: Environment variable ACC_PRESET=default
  - When: User runs `acc edit <id> --strip-tools`
...
- **TC-8.4a: CLI flag overrides environment variable**
  - Given: Environment variable ACC_PRESET=default
  - When: User runs `acc edit <id> --strip-tools=aggressive`
...
**Turn:** A turn is one user input through the final assistant response before the next user input. This matches session-cloner's definition. A "turn with tools" is any turn where the assistant used at least one tool.
...
// Built-in presets (following session-cloner's model)
...
- AgentCore installed and configured
- Node.js 18+ runtime
- File system access to agent sessions directory

Reference implementation:

- session-cloner provides patterns for CLI design, tool stripping algorithm, and output formatting

---

## Recommended Story Breakdown

The stories sequence from foundational infrastructure through discovery, core operations, and finally polish. Each story delivers a usable increment.

```
Story 0: Infrastructure
- Types, error classes, JSONL parser, test fixtures, CLI scaffold

Story 1: Session Discovery & Analysis
- ACs: AC-1.1 through AC-1.9, AC-2.1 through AC-2.7
- User can list and inspect sessions

Story 2: Edit with Tool Stripping
- ACs: AC-3.1 through AC-3.8, AC-5.1 through AC-5.8
- User can edit sessions in place with tool call stripping

Story 3: Clone Operation
- ACs: AC-4.1 through AC-4.10
- User can clone sessions with optional tool stripping

Story 4: Backup & Restore
- ACs: AC-6.1 through AC-6.5
- User can restore from automatic backups

Story 5: Output, Help & Configuration
- ACs: AC-7.1 through AC-7.8, AC-8.1 through AC-8.4
- Polished output formatting, help text, config file support
```
...
- **AgentCore Integration (future)** — Embedding compression as configurable params in AgentCore core
- **session-cloner** — Sister tool for Claude Code sessions; shares design philosophy and patterns
...
- [x] Flows cover all paths (list, info, edit, clone, restore, error recovery)
...
- [x] ACs and TCs nested under their parent flows
- [x] Cross-cutting concerns have dedicated sections
- [x] Scope boundaries are explicit
- [x] Assumptions documented with validation status and owners
- [x] Technical unknowns resolved and folded into assumptions

### Structure

- [x] Story breakdown covers all ACs
- [x] Stories sequence logically
- [x] Data contracts defined for all JSON outputs
...
- [x] Uses turns (not percentages) for tool stripping per session-cloner learnings
...
- [x] Data contract shapes realistic (AgentCore-specific types defined in Tech Design)
...
- [x] session-cloner patterns applicable (algorithm transfers, field names differ)
