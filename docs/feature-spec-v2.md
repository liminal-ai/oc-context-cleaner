...
After this feature ships, an OpenClaw agent or human operator can reduce context pressure on a running session by stripping old tool calls from the session transcript. The tool operates directly on the JSONL session files that OpenClaw uses, with edit-in-place as the primary mode (the agent continues in the same session) and clone as a fallback (a new session is created from the cleaned transcript). Before cleaning, users can list available sessions and analyze their composition to choose the right stripping preset. Edit operations are automatically backed up and restorable. Both human-readable and JSON output modes are supported, making the tool equally usable by operators at a terminal and agents invoking it programmatically.
...
| A10 | JSONL format uses toolCall/toolResult (not tool_use/tool_result), linear transcript | Validated | Tech Lead | Algorithm logic from ccs-cloner transfers; field names differ. Session header is separate `{type: "session"}` entry |
| A11 | Agent directory at ~/.clawdbot/agents/{agentId}/sessions/, default agent "main" | Validated | Tech Lead | Override via `--agent` flag or `OPENCLAW_AGENT_DIR` env var |
| A12 | Current session detection via most-recently-modified .jsonl heuristic | Validated | Tech Lead | `ls -t *.jsonl | head -1`; confirmed by Molt as expected approach. `--session <id>` overrides when specified |
...
Understanding what is in a session — how many messages, how many tool calls, how large it is — determines which stripping preset to use. Running `occ info` before `occ edit` is the expected pattern: analyze first, then act with the right level of aggression.
...
Editing in place is the primary workflow because it lets the agent continue in the same session — no context switch, no new session ID, no disruption. The agent invokes `occ edit`, the tool backs up the session, strips old tool calls, writes the cleaned transcript back, and OpenClaw picks up the changes on its next cache refresh (45s TTL with mtime check). This is the routine self-maintenance path.
...
Configuration support lets users customize behavior without passing flags on every invocation. The precedence chain — CLI flags override environment variables, which override config file values — follows standard CLI conventions and matches the ccs-cloner pattern.
